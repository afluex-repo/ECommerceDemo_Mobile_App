...
import afluex.mlm.demo.responseProductDetails.LstramItem;
...
public class AdapterRam extends RecyclerView.Adapter<AdapterRam.MyViewHolder> {
    ...
    private List<LstramItem> productList;
    SelectedVarientsListener listener;

    public AdapterRam(Context mContext, List<LstramItem> productList, SelectedVarientsListener listener) {
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, @SuppressLint("RecyclerView") int position) {
        holder.tvCategory.setText(productList.get(position).getRamName());
        ...
        holder.tvCategory.setOnClickListener(view -> {
            if (!productList.get(position).getStatus().contains("selected"))
                listener.selectedVarients("", "", "", productList.get(position).getRamID(), "", "");
        });
    }
