...
import afluex.mlm.demo.responseProductDetails.LstcolorItem;
...
public class AdapterColor extends RecyclerView.Adapter<AdapterColor.MyViewHolder> {
    ...
    private List<LstcolorItem> productList;
    SelectedVarientsListener listener;

    public AdapterColor(Context mContext, List<LstcolorItem> productList, SelectedVarientsListener listener) {
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, @SuppressLint("RecyclerView") int position) {
        holder.tvCategory.setText(productList.get(position).getColorName());
        ...
        holder.tvCategory.setOnClickListener(view -> {
            if (!productList.get(position).getStatus().contains("selected"))
                listener.selectedVarients("", productList.get(position).getColorID(), "", "", "", "");
        });
    }
