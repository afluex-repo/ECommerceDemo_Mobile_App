...
import afluex.mlm.demo.responseProductDetails.LstsizeItem;
...
public class AdapterSize extends RecyclerView.Adapter<AdapterSize.MyViewHolder> {
    ...
    private List<LstsizeItem> productList;
    SelectedVarientsListener listener;

    public AdapterSize(Context mContext, List<LstsizeItem> productList, SelectedVarientsListener listener) {
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, @SuppressLint("RecyclerView") int position) {
        holder.tvCategory.setText(productList.get(position).getSizeName());
        ...
        holder.tvCategory.setOnClickListener(view -> {
            if (!productList.get(position).getStatus().contains("selected"))
                listener.selectedVarients(productList.get(position).getSizeID(), "", "", "", "", "");
        });
    }
